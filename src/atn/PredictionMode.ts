...
export const PredictionMode = {
     ...
     * @param configs
     */
    hasSLLConflictTerminatingPrediction (mode: any, configs: any) {
     ...
     */
    hasConfigInRuleStopState (configs: any) {
        for (let i = 0; i < configs.items.length; i++) {
     ...
     * {@link RuleStopState}, otherwise {@code false}
     */
    allConfigsInRuleStopStates (configs: any) {
        for (let i = 0; i < configs.items.length; i++) {
            const c = configs.items[i];
            if (!(c.state instanceof RuleStopState)) {
                return false;
     ...
     * @param altsets
     */
    resolvesToJustOneViableAlt (altsets: any) {
     ...
     */
    allSubsetsConflict (altsets: any) {
     ...
     * {@link BitSet//cardinality cardinality} 1, otherwise {@code false}
     */
    hasNonConflictingAltSet (altsets: any) {
     ...
     * {@link BitSet//cardinality cardinality} &gt; 1, otherwise {@code false}
     */
    hasConflictingAltSet (altsets: any) {
        for (let i = 0; i < altsets.length; i++) {
            const alts = altsets[i];
            if (alts.length > 1) {
                return true;
     ...
     * @param configs
     */
    getConflictingAltSubsets (configs: any) {
    ...
    hasStateAssociatedWithOneAlt (configs: any) {
    ...
    getSingleViableAlt (altsets: any) {
        let result = null;
        for (let i = 0; i < altsets.length; i++) {
            const alts = altsets[i];
